...
#     # 소수점 자리수 처리
#     for i in range(0, int(value) + step, step):
#         formatted_value = f"{i:,.{decimal_places}f}"
#         placeholder.markdown(
#             f"""
#             <div style="border: 2px solid #ccc; border-radius: 10px; padding: 20px; text-align: center; background-color: #f9f9f9;">
#                 <div style="font-size: 18px; font-weight: bold; color: #666;">{key}</div>
...
#     # 전체 데이터 계산
#     total = len(df)
#     abnormal = df[['등외품', '재투입', '불량']].max(axis=1).sum()
#     normal = total - abnormal
#     abnormal_rate = (abnormal / total) * 100

#     weight = df['중량 (g)'].sum()
#     df_normal = df[df[['등외품', '재투입', '불량']].max(axis=1) == 0]
#     weight_normal = df_normal['중량 (g)'].sum()
...
#         animate_number(round(abnormal_rate, 2), "불량률", suffix="%")
#     with col5:
#         animate_number(weight / 1000, "투입 중량", suffix="kg", decimal_places=2)
#     with col6:
#         animate_number(weight_normal / 1000, "선별 완료 중량", suffix="kg", decimal_places=2)
#     with col7:
#         animate_number(67, "선별 속도", suffix="개/분")
...
#         # 농가 리스트 추출
#         farm_list = df['농가 번호'].unique()
#         selected_farm = st.selectbox("농가 선택", farm_list)

#         # 선택된 농가의 데이터 필터링
#         df_farm = df[df['농가 번호'] == selected_farm]
#         farm_number = df_farm['농가 번호'].iloc[0]
#         farm_name = df_farm['농가 명'].iloc[0]
#         arrival_date = df_farm['입고일자'].iloc[0]
...
#                     </div>
#                     """
#         st.markdown(basic_information, unsafe_allow_html=True)

#         # 4/5/6년근 인삼 별 대/중/소 비율 원형 차트
#         grade_counts = df_farm['등급 판정 결과'].value_counts()
#         four_year = [grade_counts.get(f"4년근 {size}", 0) for size in ["소", "중", "대"]]
#         five_year = [grade_counts.get(f"5년근 {size}", 0) for size in ["소", "중", "대"]]
#         six_year = [grade_counts.get(f"6년근 {size}", 0) for size in ["소", "중", "대"]]
...
#             st.subheader("4년근")
#             fig, ax = plt.subplots()
#             ax.pie(four_year, labels=["대", "중", "소"], autopct="%1.1f%%", startangle=90, colors=purple_colors[:3])
#             ax.axis("equal")
#             st.pyplot(fig)
...
#             st.subheader("5년근")
#             fig, ax = plt.subplots()
#             ax.pie(five_year, labels=["대", "중", "소"], autopct="%1.1f%%", startangle=90, colors=purple_colors[:3])
#             ax.axis("equal")
#             st.pyplot(fig)
...
#             st.subheader("6년근")
#             fig, ax = plt.subplots()
#             ax.pie(six_year, labels=["대", "중", "소"], autopct="%1.1f%%", startangle=90, colors=purple_colors[:3])
#             ax.axis("equal")
#             st.pyplot(fig)

#         # 연근 별 크기 선별 현황 Bar Chart
#         years = ['4년근', '5년근', '6년근']
#         sizes = ['소', '중', '대']
#         grouped_data = {size: [df_farm['등급 판정 결과'].str.contains(f"{year} {size}").sum() for year in years] for size in sizes}
#         df_bar = pd.DataFrame(grouped_data, index=years)
...
#         with cols[2]:
#             st.write("### 불량 분석 원형 그래프")
#             fig1, ax1 = plt.subplots()
#             status_counts = {"등외품": df_farm["등외품"].sum(), "재투입": df_farm["재투입"].sum(), "불량": df_farm["불량"].sum()}
#             labels = list(status_counts.keys())
#             sizes = list(status_counts.values())
#             ax1.pie(sizes, labels=labels, autopct="%.1f%%", startangle=90, colors=purple_colors[:3])
#             ax1.set_title("불량 데이터")
#             st.pyplot(fig1)
# else:
#     st.info("CSV 파일을 업로드해주세요.")
