...
# 바 차트 및 파이 차트 색상 설정 (연보라 계열)
color_palette = ["#D8BFD8", "#BA55D3", "#9370DB", "#8A2BE2", "#6A5ACD"]
...
if uploaded_file is not None:
    ...
    fig, ax = plt.subplots()
    ax.pie(
        [grade_counts.get(f"4년근 {size}", 0) for size in ["소", "중", "대"]],
        labels=["대", "중", "소"],
        autopct="%1.1f%%",
        startangle=90,
    ...
    # 가로 막대 그래프
    fig2, ax2 = plt.subplots()
    df_bar.plot(kind="barh", stacked=True, ax=ax2, color=color_palette)
    ax2.set_xlabel("개수", fontproperties=font_manager.FontProperties(fname=font_path))
    ax2.set_ylabel("연근", fontproperties=font_manager.FontProperties(fname=font_path))
    ax2.set_title("연근 별 크기 분포", fontproperties=font_manager.FontProperties(fname=font_path))
    st.pyplot(fig2)
    ...
    ax3.pie(
        [df["등외품"].sum(), df["재투입"].sum(), df["불량"].sum()],
        labels=["등외품", "재투입", "불량"],
        autopct="%.1f%%",
        startangle=90,
        colors=color_palette,
    )
    ax3.set_title("불량 데이터 분포", fontproperties=font_prop)
    st.pyplot(fig3)
else:
    st.info("CSV 파일을 업로드해주세요.")
